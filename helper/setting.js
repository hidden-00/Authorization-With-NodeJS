const settingSchema = require('../modules/setting/settingSchema');
...
module.exports.getSetting = async (type, sub_type, key) => {
    try {
        ...
        if (temp) {
            ...
            if (value == undefined) {
                const setting = await settingSchema.findOne({ key: key, type: type, sub_type: sub_type });
                if (setting) {
...
module.exports.initSettings = async () => {
    try {
        const s = await settingSchema.find({ is_deleted: false }, { value: 1, type: 1, sub_type: 1, _id: 1, key:1 }).sort({ type: 1, sub_type: 1, key: 1 }).lean();
        // console.log(s);
